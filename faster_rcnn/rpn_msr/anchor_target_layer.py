...
def anchor_target_layer(rpn_cls_score, gt_boxes, gt_ishard, dontcare_areas,
    ...
    """
    _anchors = generate_anchors(ratios=np.array(anchor_ratios), scales=np.array(anchor_scales))
    ...
    bbox_outside_weights = bbox_outside_weights.reshape((1, height, width, A * 4)).transpose(0, 3,
                                                                                             1, 2)
